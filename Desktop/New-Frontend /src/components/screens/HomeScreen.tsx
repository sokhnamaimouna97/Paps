...
import { Store, Truck, ShoppingBag, Shield, ArrowRight, Play, Zap, ShieldCheck, Clock, ChevronRight, Quote } from 'lucide-react';
import RoleCard from './RoleCard';
import { Card, CardContent } from '../ui/card';
import { Button } from '../ui/button';

interface HomeScreenProps {
  onNavigate: (view: 'merchant' | 'delivery' | 'customer' | 'paps' | 'login') => void;
}
    ...
    <div className="home-screen min-h-screen bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-[#E8F6F5] via-white to-white dark:from-[#0B1A1A] dark:via-[#0E131A] dark:to-[#0E131A]">
      <div className="max-w-6xl mx-auto px-6 pt-12 pb-10">
        ...
        <nav className={`micro-header ${shrunk ? 'shrink' : ''} sticky top-0 left-0 right-0 -mx-6 px-6 py-2 mb-6`}>
          <div className="max-w-6xl mx-auto flex items-center justify-between gap-4 text-sm">
            ...
            <div className="hidden sm:block">
              <Button size="sm" onClick={() => onNavigate('login')} className="h-8 px-3 bg-[#5CBCB6] hover:bg-[#4AA9A3] text-white">Démarrer</Button>
            </div>
          </div>
        </nav>
        <header className="text-center mb-14 relative" aria-labelledby="site-title">
          ...
          <p className="text-base md:text-lg text-[#5A6A7A] dark:text-zinc-300 max-w-xl mx-auto">Créez votre boutique en ligne et gérez vos livraisons simplement. Trois espaces, un écosystème.</p>

          {/* CTAs */}
          <div className="mt-8 flex items-center justify-center gap-3 hero-cta">
            <Button onClick={() => onNavigate('login')} className="bg-[#5CBCB6] hover:bg-[#4AA9A3] text-white shadow-md">
              Démarrer gratuitement
              <ArrowRight className="w-4 h-4 ml-2" />
            </Button>
            <Button variant="outline" onClick={() => onNavigate('customer')} className="border-[#2C3E50] text-[#2C3E50] dark:text-white dark:border-white">
        ...
        <main className="px-0">
          ...
          <section id="marche" className="mt-16" aria-label="Comment ça marche">
            <h3 className="section-title text-lg font-semibold text-center text-[#1F2D3D] dark:text-white">Comment ça marche</h3>
